...
def plot_token_val():
    ...
    for run_id in run_ids:
        if run_id[1][:2] == 'se':
            ...
            linestyle = '-'
            color = '#2ca02c'
        ser = get_series(key_prefix='val', key_suffix='selective_desc_acc_masked_only', run_id=run_id[0], load=True, update=update)
    ...
    for run_id in run_ids:
        if run_id[1][:2] == 'se':
            ...
            linestyle = '-'
            color = '#2ca02c'
        ser = get_series(key_prefix='val', key_suffix='selective_desc_class_acc', run_id=run_id[0], load=True, update=update)
...
def stabilizing_plot():
    ...
    for run_id in run_ids:
        if run_id[1][:2] == 'se':
            ...
            linestyle = '-'
        ser = get_series(key_prefix='val', key_suffix='selective_desc_class_acc', run_id=run_id[0], load=True, update=False)
...
def plot_masking_ratios_2(rnd_id=None, sel_id=None, rnd_sel_id=None):
    ...
    axs[0][0].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_rnd_code_acc_m.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[0][0].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_sel_code_acc_m.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[0][0].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_add_code_acc_m.pt').flip(0), label='additive', linestyle='dotted', color='#1f77b4')
    axs[0][0].set_ylim(0.2, 1)
    axs[0][0].set_ylabel('error rate', fontsize=fontsize)
    axs[0][0].grid(True)
    axs[0][0].tick_params(axis='x', bottom=False, labelbottom=False)
    ...
    # axs[0][0].set_title('Random Transformer', fontsize=fontsize)
    axs[1][0].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_rnd_mse.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[1][0].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_sel_mse.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[1][0].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_add_mse.pt').flip(0), label='additive', linestyle='dotted', color='#1f77b4')
    axs[1][0].set_ylim(0, 0.3)
    axs[1][0].set_ylabel('MSE', fontsize=fontsize)
    axs[1][0].grid(True)
    axs[1][0].tick_params(axis='x', bottom=False, labelbottom=False)
    ...
    axs[2][0].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_rnd_acc.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[2][0].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' /  f'{rnd_id}_sel_acc.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[2][0].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_id}_add_acc.pt').flip(0), label='additive', linestyle='dotted', color='#1f77b4')
    axs[2][0].set_ylim(0.2, 1)
    axs[2][0].set_ylabel('error rate', fontsize=fontsize)
    axs[2][0].grid(True)
    # axs[2][0].set_title('image classification')
    axs[2][0].set_xlabel('masking ratio', fontsize=fontsize)
    axs[2][0].tick_params(axis='both', which='both', labelsize=fontsize)


    axs[0][1].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_rnd_code_acc_m.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[0][1].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_sel_code_acc_m.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[0][1].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' /  f'{sel_id}_add_code_acc_m.pt').flip(0), label='additive', linestyle='dotted', color='#1f77b4')
    axs[0][1].set_ylim(0.2, 1)
    axs[0][1].grid(True)
    axs[0][1].tick_params(axis='y', left=False, labelleft=False)
    axs[0][1].tick_params(axis='x', bottom=False, labelbottom=False)
    ...
    # axs[0][1].set_title('Selective Transformer', fontsize=fontsize)
    axs[1][1].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_rnd_mse.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[1][1].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_sel_mse.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[1][1].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_add_mse.pt').flip(0), label='additive',linestyle='dotted', color='#1f77b4')
    ...
    axs[2][1].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_rnd_acc.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[2][1].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_sel_acc.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[2][1].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{sel_id}_add_acc.pt').flip(0), linestyle='dotted', color='#1f77b4')

    axs[2][1].set_ylim(0.2, 1)
    axs[2][1].grid(True)
    axs[2][1].set_xlabel('masking ratio', fontsize=fontsize)
    axs[2][1].tick_params(axis='y', left=False, labelleft=False)
    axs[2][1].tick_params(axis='both', which='both', labelsize=fontsize)


    axs[0][2].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_sel_code_acc_m.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[0][2].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_rnd_code_acc_m.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[0][2].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_add_code_acc_m.pt').flip(0), label='additive', linestyle='dotted', color='#1f77b4')
    ...
    # axs[0][0].set_title('Random Transformer', fontsize=fontsize)
    axs[1][2].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_rnd_mse.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[1][2].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_sel_mse.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[1][2].plot(torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_add_mse.pt').flip(0), linestyle='dotted', color='#1f77b4')
    axs[1][2].set_ylim(0, 0.3)
    # axs[1][0].set_title('reconstruction')
    # axs[1][0].set_ylabel('MSE', fontsize=fontsize)
    axs[1][2].grid(True)
    ...
    axs[2][2].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_rnd_acc.pt'), label='random', linestyle='solid', color='#2ca02c')
    axs[2][2].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' /  f'{rnd_sel_id}_sel_acc.pt'), label='selective', linestyle='dashed', color='#ff7f0e')
    axs[2][2].plot(1 - torch.load(util.DATA_DIR / 'mask_ratio_eval' / f'{rnd_sel_id}_add_acc.pt').flip(0), label='additive', linestyle='dotted', color='#1f77b4')
    axs[2][2].set_ylim(0.2, 1)
    axs[2][2].grid(True)
    # axs[2][0].set_title('image classification')
    axs[2][2].set_xlabel('masking ratio', fontsize=fontsize)
    axs[2][2].tick_params(axis='y', left=False, labelleft=False)
    axs[2][2].tick_params(axis='both', which='both', labelsize=fontsize)
    ...
    axs[0, 0].annotate('random transformer', xy=(0.5, 1.1), xycoords='axes fraction',
                       fontsize=fontsize, ha='center', va='center', fontweight='bold')
    axs[0, 1].annotate('selective transformer', xy=(0.5, 1.1), xycoords='axes fraction',
                       fontsize=fontsize, ha='center', va='center', fontweight='bold')

    axs[0, 2].annotate('random-selective transformer', xy=(0.5, 1.1), xycoords='axes fraction',
                       fontsize=fontsize, ha='center', va='center', fontweight='bold')
...
if __name__ == "__main__":
    # util.set_seed(0)
    # good ides
    # plot_recons_2(transformer='VQVAET-512', img_id_1=12, img_id_2=10) # 512 is rnd
    # plot_recons_2(transformer='VQVAET-512', img_id_1=8, img_id_2=9) # 512 is rnd
    # plot_recons_2(transformer='VQVAET-512', img_id_1=33, img_id_2=19) # 512 is rnd
    # plot_recons_2(transformer='VQVAET-512', img_id_1=56, img_id_2=56) # 512 is rnd

    # plot_recons_2(transformer='VQVAET-512', img_id_1=19, img_id_2=56) # 512 is rnd

    # plot_recons_2(transformer='VQVAET-511', img_id_1=19, img_id_2=56) # 512 is rnd
